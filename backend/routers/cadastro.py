...
async def listar_cadastros(
    ...
    filtro: str = Query("", description="Filtro de busca (nome ou email)"),
    ...
    try:
        ...
        cadastros = (
            query
            .order_by(models.Cadastro.data_criacao.desc(), models.Cadastro.nome.asc())
            .offset(skip)
            .limit(limit)
            .all()
        )
        
        # Processar cadastros para resposta
        ...
        for cadastro in cadastros:
            try:
                ...
                # Incluir cadastro básico mesmo com erro
...
async def atualizar_cadastro(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao atualizar cadastro: {str(e)}")
...
async def excluir_cadastro(cadastro_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao excluir cadastro: {str(e)}")
...
async def obter_estatisticas_cadastros(db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            este_mes = db.query(func.count(models.Cadastro.id)).filter(
                func.date(models.Cadastro.data_criacao) >= inicio_mes
            ).scalar()
            
            mes_anterior = db.query(func.count(models.Cadastro.id)).filter(
        ...
        return {
            ...
            "crescimento": ((este_mes - mes_anterior) / mes_anterior * 100) if mes_anterior > 0 else 0
        }
